...
local site     = base:new({
    ...
    name = "altyazidb",
    url  = "https://altyazidb.com/api/v1"
})
...
function site:getPage(queryParams)
    ...
    if queryParams.imdbId then
        ...
        content = request:timeout(15):headers({["X-API-Key"] = config.api_altyazidb}):sendRequest(self.url.."/search", {imdb_id = queryParams.imdbId, lang = queryParams.tags.language, season = queryParams.tags.s, episode = queryParams.tags.e})
    end
...
function site:download(subtitle, savePath)
    ...
    request:timeout(30):headers({["X-API-Key"] = config.api_altyazidb}):download(savePath):sendRequest(self.url.."/download", {sub_id = subtitle.id})
end
